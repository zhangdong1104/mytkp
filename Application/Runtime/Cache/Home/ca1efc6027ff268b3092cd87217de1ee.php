	...
	<head>
		...
		$(function(){
			...
			$('#dg').datagrid({
			    striped:true,
			    method: "GET",
			    url:'http://localhost:8081/mytkp/index.php/Home/Menu/loadMenuByPage?pageNo=1&pageSize=10',
			    pagination:true,
			    rownumbers:true,
			    frozenColumns:[[
			    ...
			    toolbar: [{
					...
					handler: function(){
						...
						$('#parentid').combobox({    
						    url:'http://localhost:8081/mytkp/index.php/Home/Menu/load12Menu',    
						    valueField:'menuid',    
						    textField:'name'   
						}); 
						$('#ff').form('reset');
					...
					handler: function(){
						...
						if(window.confirm("你真的想删除这些数据吗？")){
							...
							$.post("http://localhost:8081/mytkp/index.php/Home/Menu/deleteMenus",{
					...
					handler: function(){
						...
						//回填数据
						$.getJSON("http://localhost:8081/mytkp/index.php/Home/Menu/loadMenuByID?menuid="+row.menuid,{},function(data){
		...
		function refreshData(pageNumber,pageSize){
			$("#dg").datagrid('loading');
			$.getJSON("http://localhost:8081/mytkp/index.php/Home/Menu/loadMenuByPage?pageNo="+pageNumber+"&pageSize="+pageSize,{},function(result){
		...
		function searchClass(){
			$.post("http://localhost:8081/mytkp/index.php/Home/Menu/loadMenuByPage",{
				'pageNo'	 :1,
				'pageSize'	 :10,
				'name'  	 :$("#search-name").val(),
				'url'		 :$("#search-url").val(),
				'parentid'   :$("#search-parentid").combo("getValue")
			},function(result){
				$("#dg").datagrid('loadData',{
					rows: result.rows,
					total: result.total
			...
			<form action="" id="searchForm">
				<table>
					<tr>
						...
						<td>
							<input type="text" class="easyui-validatebox in" placeholder="菜单名称模糊查询" id="search-name">
						</td>
						<td colspan="2">
							<label>菜单地址：</label>
							<input type="text" class="easyui-validatebox in" placeholder="菜单地址查询" id="search-url">
						</td>
						...
						<td>
							<select class="easyui-combobox in2" id="search-parentid">
							    <option value="0">请选择菜单级别</option>
